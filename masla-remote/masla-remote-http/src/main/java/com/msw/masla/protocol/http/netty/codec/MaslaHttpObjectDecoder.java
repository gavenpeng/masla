 ...
 *
 * masla http protocol decoder,process frame too long
 */
public abstract class MaslaHttpObjectDecoder extends ByteToMessageDecoder {

    private static final Logger LOG = LoggerFactory.getLogger(MaslaHttpObjectDecoder.class);
    ...
    /**
     * The internal state of {@link MaslaHttpObjectDecoder}.
     ...
     */
    protected MaslaHttpObjectDecoder() {
            ...
            case READ_HEADER: try {
                ...
                switch (nextState) {
                    ...
                    default:
                        ...
                        //出现大body请求时，再聚合handler不能丢弃body，只有在这里才可以
                        MaslaHttpObjectAggregator aggregator = ctx.pipeline().get(MaslaHttpObjectAggregator.class);
                        if (aggregator != null) {
                            int maxContextLength = aggregator.maxContentLength();

                            if(contentLength> maxContextLength){

                                throw new TooLongFrameException("Request entity too large: " + contentLength);
                            }
                        }
    ...
    protected void decodeLast(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if (message != null) {
            ...
            //这里是nginx只发完了请求行，并没有发送请求头，就关闭了链接
    ...
    private HttpMessage invalidMessage(ByteBuf in, Exception cause) {
        ...
        if (message == null) {
            //如果message为null，代表请求行解析失败，之前官方的做法是直接给了一个
            //bad-request请求行，网关需要个性化处理，因为都是请求行太大，所以返回
            //解析完的那一部分.
            AppendableCharSequence line = this.lineParser.getSeq();
            try {
                String[] initialLine = splitInitialLine(line);
                message = createMessage(initialLine);
            }catch (Throwable e){
            ...
            if(cause.getMessage() != null &&
                    cause.getMessage().startsWith(Constants.NETTY_HEADER_ERROR)) {
                //代表解析header失败，一般都是出现大的头，所以这里
                //需要记录最后一个解析失败的key和value。
                try {
                    AppendableCharSequence line = this.headerParser.getSeq();
                    if (line.length() > 0) {
                        splitHeader(line);
                    ...
                    message.headers().add("err-header", this.headerParser.getSeq());
                }
    ...
    private  void setRequestSize(ChannelHandlerContext ctx){
        //增加处理请求行，请求头解析失败的情况
        if(currentState == State.READ_INITIAL){
            if(message != null) {
                if (message instanceof HttpRequest) {
                    ctx.channel().attr(MaslaChannelAttribute.REQ_LINE_SIZE).set(this.lineParser.getSize());
                } else {
                    ctx.channel().attr(MaslaChannelAttribute.RESP_LINE_SIZE).set(this.lineParser.getSize());
                }
            }else{
                //请求行/响应行太大
                if(this instanceof MaslaHttpRequestDecoder){
                    ctx.channel().attr(MaslaChannelAttribute.REQ_LINE_SIZE).set(this.lineParser.getSize());
                }else{
                    ctx.channel().attr(MaslaChannelAttribute.RESP_LINE_SIZE).set(this.lineParser.getSize());
                }
            ...
            //到解析请求头，肯定有message
            if(message instanceof HttpRequest) {
                ctx.channel().attr(MaslaChannelAttribute.REQ_HEADER_SIZE).set(this.headerParser.getSize());
            }else{
                ctx.channel().attr(MaslaChannelAttribute.RESP_HEADER_SIZE).set(this.headerParser.getSize());
            }
        }
    }
    ...
    private static String[] splitInitialLine(AppendableCharSequence sb) {
        ...
        return new String[] {
                ...
                cStart < cEnd? sb.subStringUnsafe(cStart, cEnd) : "HTTP/1.1" };
    }
    ...
    private static class HeaderParser implements ByteProcessor {
        ...
        public AppendableCharSequence parse(ByteBuf buffer) {
            ...
            seq.reset();
            /*
              buffer 读一行，读到换行符时返回，回车符内容不会写到seq里。
              所以解析请求行时，seq 是请求行的所有内容
              解析header时，seq是一个key和value的值，如果读到seq的长度为0，则代表头读完了
              size 是请求行或者所有header的key和value的大小的总和
            */
        ...
        public boolean process(byte value) throws Exception {
            ...
            if (size > maxLength) {
//                /*如果是请求行/请求头出现太大,大于阀值maxLength，目前就是只获取了前maxLength的内容，后面的内容
//                  就丢弃了，这样不能定位客户端的问题，改进后，如果大于maxLength，就扩容到2*maxLength
//                  把后面的uagent 读出来，可以定位到是ios还是android,尽量能看到一个完整的请求数据
//                */

                // 网关需要白盒化，需要对请求行太长的请求进行特殊处理，不重写请求行
                //if()
...
//                    throw newException(maxLength);
                throw newException(maxLength);
                //如果大于maxLength，后面的直接移动buffer的偏移量，不读数据，浪费内存和cpu，因为需要copy
            }
        ...
        protected AppendableCharSequence getSeq(){
            return this.seq;
        }
