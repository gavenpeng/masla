 ...
 *  p.addLast("aggregator", <b>new {@link io.netty.handler.codec.http.HttpObjectAggregator}(1048576)</b>);
 *  ...
 *  p.addLast("handler", new HttpRequestHandler());
 *  </pre>
 * </blockquote>
 * <p>
 * For convenience, consider putting a {@link HttpServerCodec} before the {@link io.netty.handler.codec.http.HttpObjectAggregator}
 * as it functions as both a {@link HttpRequestDecoder} and a {@link HttpResponseEncoder}.
 * </p>
 * Be aware that {@link io.netty.handler.codec.http.HttpObjectAggregator} may end up sending a {@link HttpResponse}:
 ...
 * @see HttpServerCodec
 */
public class MaslaHttpObjectAggregator
        extends MaslaMessageAggregator<HttpObject, HttpMessage, HttpContent, FullHttpMessage> {
    private static final InternalLogger logger = InternalLoggerFactory.getInstance(io.netty.handler.codec.http.HttpObjectAggregator.class);
     ...
     */
    public MaslaHttpObjectAggregator(int maxContentLength) {
     ...
     */
    public MaslaHttpObjectAggregator(int maxContentLength, boolean closeOnExpectationFailed) {
...
//            MaslaHttpObjectDecoder decoder = ctx.pipeline().get(MaslaHttpObjectDecoder.class);
//            if (decoder != null) {
//                decoder.reset();
//            }

            String value = oversized.headers().get(HttpHeaderNames.CONTENT_LENGTH);
            if(value == null){
                value = String.valueOf(maxContentLength());
            }
            throw new TooLongFrameException("Request entity too large: " + value);
