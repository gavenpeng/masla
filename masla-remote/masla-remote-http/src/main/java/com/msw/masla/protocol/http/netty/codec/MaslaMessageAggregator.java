 ...
 */
public abstract class MaslaMessageAggregator<I, S, C extends ByteBufHolder, O extends ByteBufHolder>
     ...
     */
    protected MaslaMessageAggregator(int maxContentLength) {
        validateMaxContentLength(maxContentLength);
        this.maxContentLength = maxContentLength;
    }

    protected MaslaMessageAggregator(int maxContentLength, Class<? extends I> inboundMessageType) {
    ...
    public final int maslaMaxContentLength(){
        return maxContentLength;
    }
    ...
    protected void decode(final ChannelHandlerContext ctx, I msg, List<Object> out) throws Exception {
        if (isStartMessage(msg)) {
            ...
            } else if (isContentLengthInvalid(m, maslaMaxContentLength())) {
                try {
                    invokeHandleOversizedMessage(ctx, m);
                }catch (Exception e){
                    HttpMessage ret = (HttpMessage)beginAggregation(m,EMPTY_BUFFER);
                    invalidMessage(ret,e);
                    out.add(ret);
                }
            ...
            if (content.readableBytes() > maslaMaxContentLength() - m.content().readableBytes()) {
                ...
                try {
                    ...
                    invalidMessage((HttpMessage)currentMessage,e);
...
//            ReferenceCountUtil.release(oversized);
    ...
    private void invalidMessage(HttpMessage msg, Exception cause) {
        // Advance the readerIndex so that ByteToMessageDecoder does not complain
        // when we produced an invalid message without consuming anything.
//        in.skipBytes(in.readableBytes());
        msg.setDecoderResult(DecoderResult.failure(cause));
    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        try {
            ...
            /**
             * 如果网关在等nginx/或者客户端的body，而出现链接关闭，就是body还没有发完，
             * 有可能是网络问题，也有可能是攻击的请求，故意不发body，导致一等直到链接关闭
             * 网关需要监控这类请求，所以如果有message而且不为null，则需要把message转发
             * 到后面到handler
             */
            fireBodyMiss();
        } finally {
    ...
    private void fireBodyMiss(){
        if (currentMessage != null) {
            HttpMessage httpMessage = (HttpMessage)currentMessage;
            int missSize = HttpUtil.getContentLength(httpMessage,-1)-currentMessage.content().readableBytes();
            missBody(httpMessage,new HttpBodyMissException("HTTP body miss:"+missSize));
    ...
    private void missBody(HttpMessage msg, Exception cause) {
        // Advance the readerIndex so that ByteToMessageDecoder does not complain
        // when we produced an invalid message without consuming anything.
//        in.skipBytes(in.readableBytes());
        msg.setDecoderResult(DecoderResult.failure(cause));
