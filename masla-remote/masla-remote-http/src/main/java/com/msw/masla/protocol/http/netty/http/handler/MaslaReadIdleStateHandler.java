 ...
 */
public class MaslaReadIdleStateHandler extends ChannelDuplexHandler {
     ...
     */
    public MaslaReadIdleStateHandler(
    ...
    /**
     * @see #MaslaReadIdleStateHandler(boolean, long, long, long, TimeUnit)
     */
    public MaslaReadIdleStateHandler(
     ...
     */
    public MaslaReadIdleStateHandler(boolean observeOutput,
                                     long readerIdleTime, long writerIdleTime, long allIdleTime,
                                     TimeUnit unit) {
        if (unit == null) {
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        // 写时，也更新read的时间，这样能准确的计算在响应的超时。
        // 如果不更新lastReadTime，那个读超时就是上次读完，到释放到连接池，到获取该连接，写数据一直到响应的时间，
        // 如果连接在池里空闲比较久，假如空闲了55s,这时再取出来，写数据，导致等5s就超时了。
        // 所以这里需要更新下读的时间,保证是发数据出去的时间
        if (readerIdleTimeNanos > 0 || allIdleTimeNanos > 0) {
            lastReadTime = ticksInNanos();
        }
    ...
    private final class ReaderIdleTimeoutTask extends AbstractIdleTask {
        ...
        protected void run(ChannelHandlerContext ctx) {
            ...
            //reading 为ture 时，说明正在读，只是没有读完，这样不会报读超时
            //reading 为false 时，说明读完了，这时就看上次读的时间,如果间隔大于nextDelay,则生成一个空闲事件
            if (!reading) {
    ...
    private final class WriterIdleTimeoutTask extends AbstractIdleTask {
        ...
        protected void run(ChannelHandlerContext ctx) {
            ...
            long lastWriteTime = MaslaReadIdleStateHandler.this.lastWriteTime;
